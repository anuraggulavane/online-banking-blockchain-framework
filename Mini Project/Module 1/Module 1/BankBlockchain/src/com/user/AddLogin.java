...
public class AddLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    try
		{
	    	...
	    	else if(roll.equals("Driver"))
	    	{
	    		Connection con=DBconn.conn();
				 Statement st = (Statement) con.createStatement();
				    ResultSet rs;
				    String str="select * from driverregistertbl where Demail='" + userid + "' and Dpassword='" + pwd + "'";														JavaX.initTransaction();
				    //System.out.println("Str=>"+str);
				    rs = ((java.sql.Statement) st).executeQuery(str);
				    if(rs.next())
				    {
				    	
				    	session.setAttribute("Did", userid);
				    	session.setAttribute("Dmobile", rs.getString("DMbNo"));
				    	session.setAttribute("Dusername", rs.getString("Dusername"));
						...
						//rd.include(request, response); 
						response.sendRedirect("DHome.jsp?succ");
				    }
				    else 
				    {
				    	pw.println("<html><script>alert('Incorrect Email Id or Password.....');</script><body>");
